import { type Story, addStory, getStories } from "@cfce/database"
...
export async function POST(req: NextRequest) {
  try {
    ...
    const flattenedTypedStory = Object.keys(story).reduce((acc, key) => {
      const value = story?.[key]?.[0] ?? 0
      switch (key) {
        case "amount":
        ...
        case "created":
          ...
          break
        default:
          // @ts-ignore TODO: find a better way to handle this
          acc[key] = value
      }
      return acc
    }, {} as Story)
    ...
    const image = files?.[0]
    const result = await createStory(flattenedTypedStory, image)
