...
export default class Web3Server extends ChainBaseClass {
  ...
  async mintNFT1155({
    ...
    const bytes = this.web3.utils.utf8ToHex(uri)
    console.log("BYTES", bytes)
    const tokenIdHash = this.web3.utils.keccak256(tokenId)
    console.log("NUMERICID", tokenIdHash)
    const data = instance.methods
      .mint(address, tokenIdHash, 1, bytes)
      .encodeABI()
    console.log("DATA", data)
