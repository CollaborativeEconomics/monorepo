...
// ONE TO ONE
export async function updateStoryLink(initiativeId:string, storyId:string, amount:string){
  const res = await sql`UPDATE donations SET "storyId" = ${storyId}, status=3 WHERE _id IN (
                          SELECT _id FROM (
                            SELECT _id, created, (
...
// ONE TO MANY
export async function updateImpactLink(initiativeId:string, storyId:string, amount:string){
