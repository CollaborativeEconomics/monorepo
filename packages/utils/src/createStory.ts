import { Story, getInitiativeById, getOrganizationById, newStory, prismaClient, updateStory } from "@cfce/database";
...
export default async function createStory({ organizationId, initiativeId, amount, ...story }: Omit<Story, 'tokenId' | 'created'>, image?: File): Promise<Story> {
  let imageCID = '';
  let tokenCID = '';
  let storyId = '';
  try {
