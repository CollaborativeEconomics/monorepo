...
class CeloServer {
  ...
  async sendPayment(address:string, amount:string, destinTag:string, callback:any){
    ...
    const secret = process.env.CELO_MINTER_WALLET_SEED || ''
    //const source = process.env.CELO_MINTER_WALLET
  ...
  async mintNFT(uri: string, address: string){
    ...
    const contract = process.env.NEXT_PUBLIC_CELO_MINTER_CONTRACT || ''
