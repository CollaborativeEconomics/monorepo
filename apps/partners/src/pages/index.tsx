...
export default function Page() {
  ...
  useEffect(() => {
    const _session = session; // stupid hack as some keys are not part of the object
    const name = _session?.data?.user?.name ?? '';
    const orgId = _session?.data?.orgId ?? '';
    let welcomeText = `Welcome ${titleCase(name)}`;
    if (!orgId) {
      welcomeText = 'You are not authorized, request access to the portal';
    }
