...
export default function Page({
  ...
  function listWallets(chain: string, network: string) {
    const wallets = organization?.wallets?.filter(
      it => it?.chain === chain && it?.network === network,
    );
  ...
  async function onSubmit(data: FormProps) {
    ...
    const url = `/dashboard/contract/${data.contract_type.toLowerCase()}?chain=${
      data.chain
    }&network=${network}&wallet=${selectedWallet}&organizationId=${
      organization.id
    }`;
    console.log('URL', url);
    redirect(url);
