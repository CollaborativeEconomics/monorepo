...
import { newCronjob } from '@/lib/database/cronjobs'
...
export default async function handler(req, res) {
  ...
  const authorized = headers['authorization'] == `Bearer ${process.env.CRON_SECRET}`
  if (!authorized) {
  ...
  switch (method) {
    case "GET":
      try {
        ...
        const result = await runHook(Triggers.onceDaily, organizationId, { walletAddress })
        ...
        // Save error to db
        console.log(error)
        const saved = await newCronjob({cron:cronName, status:2, result:{error:error.message}})
